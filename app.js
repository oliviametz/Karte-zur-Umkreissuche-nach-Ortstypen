...
const estateMainImgSrcMedium = 'images/fictional-realestate-1-medium.jpg';
/* In WP: const estateMainImgSrcMedium = document.querySelector('.carousel-item img').getAttribute('data-medium-url'); 
(<img src="<?php echo wp_get_attachment_image_url(29, 'full'); ?>" data-medium-url="<?php echo wp_get_attachment_image_url(29, 'medium'); ?>"
...
async function addEstateMarker(location) {
    ...
    const estatePin = new PinElement({
        background: '#c3aa57',
        borderColor: '#9b833e',
        glyphColor: '#9b833e',
    });

    // Create the marker with the customized PinElement
    const estateMarker = new AdvancedMarkerElement({
        map,
        position: location,
        gmpClickable: true,
        content: estatePin.element,
    });


    // Eventlistener on Estate Marker to show details

    estateMarker.addEventListener('gmp-click', () => {
        infoWindow.close();
...
function quotaErrorCheck(caughtError) {
    ...
    /* Temporarely overwriting the funcitonality of console.error to enable itterating through messages 
       (the initMap function doesn't throw a catch when the quotas regarding map loads are exhaustet but succeeds in the request while throwing an error and displaying the map, but modified to be unusable) */
